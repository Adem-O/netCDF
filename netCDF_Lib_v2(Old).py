...
def KD490_Plot_Data (file, **kwargs):
    ...
    cbar = fig.colorbar(cs,fraction=0.046, pad=0.05,ticks=[0.01,0.02,0.05,0.1,0.2,0.5,1,2,5], format=formatter)
...
def KD490_Plot_Data_Region (file, coords =[], **kwargs):
    ...
    axs[1].stock_img()
    axs[2].stock_img()
    pad_lon1 = (np.max(lon_reduced) - np.min(lon_reduced))*3
    pad_lat1 = (np.max(lat_reduced) - np.min(lat_reduced))*3
    ...
    rect = patches.Rectangle((np.min(lon_reduced),np.min(lat_reduced)), w, h, linewidth=2, edgecolor='black', facecolor='none')
    axs[1].add_patch(rect)
    rect = patches.Rectangle((np.min(lon_reduced),np.min(lat_reduced)), w, h, linewidth=2, edgecolor='black', facecolor='none')
...
def Compare_dataBinning_plot (EarliestDate,LatestDate, folderpaths=[], coords=[], **kwargs):
    ...
    axs[0].legend(loc='center left', bbox_to_anchor=(1, 0.5),
      fancybox=False,frameon=True, ncol=1,fontsize=10)
    axs[0].text(0.7,1.2,'Var ({}) = {} '.format(many_avg_1[4][0],np.round(np.nanvar(avg_1)*10**3,5))+r'$\cdot~10^{-3}~[\mathrm{m}^{-1}]$'+'\nVar ({}) = {} '.format(many_avg_2[4][0],np.round(np.nanvar(avg_2)*10**3,5))+r'$\cdot~10^{-3}~[\mathrm{m}^{-1}]$', ha='left', va='top', transform=axs[0].transAxes,fontsize=12, backgroundcolor='lightgrey')

    axs[1].set_title('\n Percentage of empty data points captured')
    axs[1].bar(Dates_minor,Nans_1, label = ('{} \n {} \n {}\n ({})'.format(many_avg_1[7][0],many_avg_1[5][0],
                                                                                many_avg_1[5][-1],many_avg_1[4][0])),alpha=0.7)
    axs[1].bar(Dates_major+Cent,Nans_2, label = ('{} \n {} \n {}\n ({})'.format(many_avg_2[7][0],many_avg_2[5][0],
...
#     gap = np.argwhere(Dates_major[1] == Dates_minor) - np.argwhere(Dates_major[0] == Dates_minor)
#     gap = gap[0][0]
    for i in range(len(Dates_major)+1):
        axs[0].axvline(Dates_major[0] + i*Cent*2, linestyle=(0,(4,4)), linewidth=1, alpha=0.3, color='grey')
...
#         a = i*gap
#         b = gap+i*gap
#         var = np.round(np.nanvar(avg_1[a:b])*10**3,5)
#         axs[0].text(Dates_major[i]+ Cent/2, np.nanmax(avg_1)*1.1, 'Var = {}'.format(var), fontsize=8)
